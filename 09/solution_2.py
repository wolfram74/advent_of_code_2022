...
def construct_tail_path(head_path):
    # tail_path = [(0,0),]
    tails = [[(0,0),] for el in range(9)]
    # print(tail_path)
    for step in range(1,len(head_path)):
        reference = head_path[step]
        for tail_num, tail in enumerate(tails):
            if tail_num > 0:
                reference = tails[tail_num-1][-1]
            tails[tail_num].append(
                tail_response(reference, tail[-1])
                )
    return tails[-1]
...
if __name__ == '__main__':
    ...
    # with open('test_input.txt', 'r') as head_moves: # results in 1
    # with open('test_input2.txt', 'r') as head_moves: # results in 36
