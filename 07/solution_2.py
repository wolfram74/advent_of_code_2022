...
def files_over_threshold(folder_dict, threshold):
    caniddates = []
    for address in folder_dict.keys():
        folder = folder_dict[address]
        if folder.size >= threshold:
            caniddates.append(( folder.size, folder.address))
    print(caniddates)
    print(sorted(caniddates))
...
if __name__ == '__main__':
    with open('input.txt', 'r') as command_log:
        ...
        print(folder_dict['/'].size)
        drive_size = 70*(10**6)
        space_needed = 30*(10**6)
        deficit = space_needed - (drive_size-(folder_dict['/'].size))
        files_over_threshold(folder_dict, deficit)
